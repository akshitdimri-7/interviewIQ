...
export const submitAnswer = async (req, res) => {
  try {
    ...
    const question = interview.questions[questionIdx];

    if (!answer) {
      question.score = 0;
      question.feedback = "You didn't submit the answer.";
      question.answer = "";
    ...
    if (timeTaken > question.timeLimit) {
      question.score = 0;
      question.feedback = "Submit answers within the time frame.";
...
export const finishInterview = async (req, res) => {
  try {
    ...
    if (!interview) {
      return res.status(400).json({ messgae: "Failed to find interview." });
    }
...
export const getMyInterview = async (req, res) => {
  try {
    const interview = await InterviewPage.findOne({ userId: req.userId })
      ...
      .select("role experiencd mode finalScore status createdAt");

    return req.status(200).json(interviews);
  } catch (error) {
    return res
      .status(500)
      .json({ message: `Interview History error:${error}` });
  }
};

export const gertInterviewReport = async (req, res) => {
  try {
